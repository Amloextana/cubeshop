...
const Cart = () => {
  ...
  const fetchCartData = async () => {
    try {
      ...
      setCartItems(response.data.components)
    } catch (e) {
      console.log(e)
    }
  }
  ...
  const deleteItem = async (itemId: number) => {
    try {
      const responce = await axios(
        `http://localhost:8000/applications/${cartApplication}/delete_component/${itemId}/`,
        {
          method: "DELETE",
          // withCredentials: true,
          // headers: {
